...
############################## DADOS ##############################
df = pd.read_csv("./data/tb_netflix.txt", sep=";")
...
st.sidebar.write("---")
cor_graficos = st.sidebar.selectbox("Escolha uma cor:", ["dark:salmon_r", "RdGy", "Reds", "Greys"])
st.sidebar.write("---")
st.sidebar.write("[![GitHub](https://img.shields.io/badge/-GitHub-333333?style=for-the-badge&logo=github)](https://github.com/luizaalovo)")
st.sidebar.write("[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/luiza-angelo-lovo)")
...
with st.container():
    ...
    sns.heatmap(df_qtd_ano_mes_added_filmes, annot=True, fmt="d", linewidths=.5, ax=ax)
    plt.xlabel(None)
    plt.ylabel(None)
    st.pyplot(fig)
    plt.clf()
    ...
    st.markdown("### Séries")
    fig, ax = plt.subplots(figsize=(9, 6))
    sns.heatmap(df_qtd_ano_mes_added_series, annot=True, fmt="d", linewidths=.5, ax=ax)
    plt.xlabel(None)
    plt.ylabel(None)
    st.pyplot(fig)
    plt.clf()
...
with st.container():
    ...
    with tab1:
        ...
        sns.histplot(filmes, ax=ax, color="red")
        plt.xlabel("Minutos")
        st.pyplot(fig)
        plt.clf()
    ...
    with tab2:
        st.markdown("### Séries")
        fig, ax = plt.subplots(figsize=(9, 6))
        sns.histplot(series, ax=ax, color="red")
        plt.xlabel("Seções")
        st.pyplot(fig)
        plt.clf()
